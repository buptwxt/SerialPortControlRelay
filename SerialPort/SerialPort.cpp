...
//处理函数  wangxintang
void ParseReceive(char *szBuf, int icmdCount, bool bRelayFlag, bool bInputFlag) 
{    
	if(icmdCount == 6)
	{		
		if(bRelayFlag)
...

{
	...
	memset(m_szWriteBuffer,0,writebuffersize);
	...
	m_hComm = CreateFile(szPort,						// communication port string (COMX)
						 ...
						 FILE_ATTRIBUTE_NORMAL|FILE_FLAG_OVERLAPPED,      //FILE_FLAG_OVERLAPPED, // Async I/O
	...
	{						   
		if (SetCommMask(m_hComm, dwCommEvents))
		{
			...
			if (GetCommState(m_hComm, &m_dcb))
			{
				...
				m_dcb.StopBits = mystop;  //注意StopBits的值一定要小心，1或者2比较好，1.5有问题。

				////wangxintang
				//m_dcb.fBinary = TRUE;    // Bianry must be TRUE in Win32  wang
				//m_dcb.fParity = FALSE;   //wang
				//m_dcb.fDtrControl = DTR_CONTROL_ENABLE;   //wang
				////m_dcb.fRtsControl = RTS_CONTROL_ENABLE;
				//m_dcb.fOutxCtsFlow = FALSE;   //wang
				...
				//m_dcb.EvtChar = 0x00;
				//m_dcb.wReserved1 = 0;
				int ret = SetCommState(m_hComm, &m_dcb);
				//if(ret)
				//	printf("**********同志们，辛苦了SetCommState: \n");
	...
	{ 
		...
		if (bRead)
		{
			bResult = ReadFile(port->m_hComm,		// Handle to COMM port 
							   &(*(szBuf+nBufSize)),				// RX Buffer Pointer
							   1,					// Read one byte
			...
			else
			{
				...
				//
				if(0xA3 == *(szBuf+nBufSize) || 0xA5 == *(szBuf+nBufSize))  /*返回命令字开始*/
				{
					if((0xA3 == *(szBuf+nBufSize)) &&  (!bRelayFlag) )
						bRelayFlag = true;
					if((0xA5 == *(szBuf+nBufSize)) &&  (!bInputFlag) )
						bInputFlag = true;
				...
				else if(0x0A == *(szBuf+nBufSize))  /*返回命令字结束*/
				{
	
					icmdCount++;

					//  回调函数处理返回的结果  wangxintang

					// pPort->pCallBack((char *)szBuf, icmdCount, bRelayFlag, bInputFlag); 
					ParseReceive((char *)szBuf, icmdCount, bRelayFlag, bInputFlag);
...
{		
	assert(m_hComm != 0 || strlen(string) > m_nWriteBufferSize);

	memset(m_szWriteBuffer, 0, strlen(string));
...
{
	assert(m_hComm != 0 || n > m_nWriteBufferSize);
